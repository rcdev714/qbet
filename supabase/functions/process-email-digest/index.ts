...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: pendingRows, error } = await adminClient.from("email_digest_queue").select("*").is("processed_at", null).order("created_at", {
      ascending: true
    }).limit(500);
    if (error) {
      return json({
    ...
    for (const row of pendingRows ?? []){
      const { data: prefs } = await adminClient.from("user_notification_preferences").select("email_frequency, email_digest_hour_utc, email_enabled").eq("user_id", row.user_id).maybeSingle();
    ...
    for (const [userId, items] of byUser.entries()){
      ...
      const { data: userRow } = await adminClient.from("users").select("email").eq("id", userId).maybeSingle();
      ...
      if (sendResult.ok) {
        ...
        await adminClient.from("email_digest_queue").update({
