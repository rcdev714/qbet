import type {
    BetContractEventType,
    BetContractRecord,
    BetContractSnapshot,
} from "./bet-contract-types.ts";
...
export function formatContractMoney(amount: number, currency = "USD"): string {
  try {
  ...
  const resolutionBlock =
    resolution && params.eventType === "resolved"
      ? `<section style="margin-top:24px;">
          ...
          <p><strong>Payout:</strong> ${escapeHtml(formatContractMoney(Number(resolution.payoutAmount ?? 0), snapshot.wallet.currency))}</p>
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;line-height:1.6;color:#0f172a;max-width:720px;margin:0 auto;padding:24px;">
  ...
  <section>
    ...
    <p><strong>Debit:</strong> ${escapeHtml(formatContractMoney(Number(snapshot.wallet.debitAmount), snapshot.wallet.currency))}</p>
  </section>
  ...
  <section style="margin-top:24px;">
    ...
    <p><strong>Stake:</strong> ${escapeHtml(formatContractMoney(Number(snapshot.position.amount), snapshot.wallet.currency))}</p>
...
export function buildBetContractPlainText(params: {
  ...
  const lines = [
    ...
    `Option: ${snapshot.position.optionLabel} (${snapshot.position.side.toUpperCase()})`,
    `Stake: ${formatContractMoney(Number(snapshot.position.amount), snapshot.wallet.currency)}`,
  ];
  if (resolution && params.eventType === "resolved") {
    lines.push(`Outcome: ${resolution.outcome.toUpperCase()}`);
    lines.push(`Payout: ${formatContractMoney(Number(resolution.payoutAmount ?? 0), snapshot.wallet.currency)}`);
  }
  lines.push(`View: ${appBase}/contract/${params.contract.bet_id}`);
  return lines.join("\n");
}

export function buildBetContractAttachmentFilename(
  contractNumber: string,
  eventType: BetContractEventType,
  ext: "pdf" | "html",
): string {
  const phase = eventType === "placed" ? "placed" : "settled";
  const safe = contractNumber.replace(/[^a-zA-Z0-9_-]/g, "-");
  return `Anymarkt-Wager-${safe}-${phase}.${ext}`;
}
