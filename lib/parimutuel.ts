...
export interface YesNoPayoutCalculation {
  userBet: number;
  price: number;
  grossPayout: number;
  netPayout: number;
...
export function calculateImpliedOdds(options: MarketOption[]): ImpliedOdds[] {
  const totalPool = options.reduce(
    (sum, opt) => sum + Number(opt.total_pool),
    0,
  );
...
/**
 * Calculate YES/NO prices for a binary market option
 * @param yesPool Total pool on YES side
 * @param noPool Total pool on NO side
 * @returns YES/NO prices where yesPrice + noPrice = 1
 */
export function calculateYesNoPrice(
  yesPool: number,
  noPool: number,
): YesNoPrice {
  const totalPool = Number(yesPool) + Number(noPool);

  // No bets yet - default to 50/50
  if (totalPool <= 0) {
    return { yesPrice: 0.5, noPrice: 0.5, totalPool: 0 };
  }

  // Calculate raw prices
  let yesPrice = Number(yesPool) / totalPool;
  let noPrice = Number(noPool) / totalPool;

  // Clamp to min 1% / max 99% to avoid 0¢ or 100¢ extremes
  ...
  return { yesPrice, noPrice, totalPool };
}

/**
 * Calculate payout for YES/NO share price
 * @param betAmount Amount user wants to bet
 * @param price YES or NO price (0-1)
 * @param vig House edge (0-1)
 */
export function calculateYesNoPayout(
  betAmount: number,
  price: number,
  vig: number = 0.10,
): YesNoPayoutCalculation {
  if (price <= 0) {
    return {
      userBet: betAmount,
      price,
  ...
  const grossPayout = betAmount / price;
  const grossProfit = grossPayout - betAmount;
  // Apply vig only to the profit, not the original stake
  const netProfit = grossProfit * (1 - vig);
  const netPayout = betAmount + netProfit;
  const potentialProfit = netProfit;
